/**
 * sessionStorage-based cache implementation
 */

import { CacheEntry, CacheStats, CacheBackend } from './cache-types';

export class SessionStorageCache implements CacheBackend {
  private readonly prefix: string;
  private readonly maxSize: number;
  private readonly ttl: number;
  private readonly namespace: string;

  constructor(maxSize: number = 50, ttl: number = 3600000, namespace: string = 'session') {
  ...
  async get<T = any>(key: string): Promise<T | null> {
    try {
      ...
      console.error(`SessionStorageCache.get error for key ${key}:`, error);
      return null;
    }
  }
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'QuotaExceededError') {
        ...
        try {
          ...
          console.error(`SessionStorageCache.set retry error for key ${key}:`, retryError);
          return false;
        }
      }
      console.error(`SessionStorageCache.set error for key ${key}:`, error);
      return false;
    }
  }
  ...
  async delete(key: string): Promise<boolean> {
    try {
      ...
      console.error(`SessionStorageCache.delete error for key ${key}:`, error);
      return false;
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      console.error('SessionStorageCache.clear error:', error);
    }
  }
  ...
  async keys(): Promise<string[]> {
    ...
    try {
      ...
      console.error('SessionStorageCache.keys error:', error);
    }
  ...
  async stats(): Promise<CacheStats> {
    ...
    return {
      hits: hitCount,
      misses: 0, // We don't track misses in sessionStorage
  ...
  // Redis-like methods (adapted for sessionStorage)
  ...
  private async evictEntries(): Promise<void> {
    ...
    for (let i = 0; i < Math.min(toRemove, keyAges.length); i++) {
      sessionStorage.removeItem(keyAges[i].key);
    }
  }
